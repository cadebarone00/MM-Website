...
- **Named, tracked limitation (found in final review, ruled out of scope for this sub-phase):** a round never advances to the next one automatically — `currentRoundForPlayer.ts` has no real score snapshot to detect a match box going Final, and nothing sets `live_match_boxes.state = 'Final'` on submission. Per the master ops spec's own phasing, round completion/advancement is Tiger's official-review-and-settlement responsibility (sub-phase 2), not raw player submission — so a single round works end to end, but a multi-round tournament will stay pinned to round 1 until sub-phase 2 ships. Not a corner cut in this phase; genuinely belongs to the next one.
