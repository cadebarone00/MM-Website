...
export function CourseApiSearch({ configured, courses, target, onSaved, onCancel }: { configured: boolean; courses: LiveCourse[]; target?: LiveCourse; onSaved: (course: LiveCourse) => void; onCancel?: () => void }) {
  ...
  async function importCourse(result: GolfSearchResult) {
    setBusy(true); setError(null); setMessage("");
    try {
      const courseId = target?.id ?? existingId;
      const response = await fetch("/api/portal/tiger/courses/provider", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ providerId: result.id, ...(courseId ? { courseId } : {}) }) });
      const data = await response.json();
      if (!response.ok || !data.ok) throw new Error(data.error ?? "Import failed.");
      onSaved(data.course); setExistingId(""); setMessage(`${data.course.name} saved. Review the imported tee sets and lock them when ready.`);
  ...
  return <section className="mt-4 rounded-lg border border-gold-300 bg-white p-4">
    ...
    {target && <p className="mt-2 text-xs text-ink-500">Matching tee names are linked in place; new tees are added. Your existing values and saved score references are preserved.</p>}
    {!target && <label className="mt-3 block text-sm text-ink-700">Save to existing course
      <select value={existingId} disabled={busy} onChange={(event) => setExistingId(event.target.value)} className="mt-1 block w-full rounded border border-stone-300 px-3 py-2">
        <option value="">Create a new course</option>
        {courses.filter((course) => !course.teeSets?.some((tee) => tee.apiSource?.provider === "golfcore")).map((course) => <option key={course.id} value={course.id}>{course.name}</option>)}
      </select>
      <span className="mt-1 block text-xs text-ink-500">If this course is already in your library, select it here to avoid a duplicate.</span>
    </label>}
    ...
    <div className="mt-3 divide-y divide-stone-200">{results.map((result) => {
      const saved = courses.some((course) => course.teeSets?.some((tee) => tee.apiSource?.provider === "golfcore" && tee.apiSource.courseId === result.id));
      return <div key={result.id} className="flex items-center justify-between gap-3 py-3"><div><h3 className="font-bold">{result.name}</h3><a href={golfCoreUrl(result.id)} target="_blank" rel="noreferrer" className="text-xs underline">Course data: GolfCore</a><p className="text-xs text-ink-500">{result.location} · {result.holes} holes</p></div><button type="button" disabled={busy || saved || result.holes !== 18} onClick={() => importCourse(result)} className="shrink-0 rounded border border-maroon-700 px-3 py-2 text-xs font-bold text-maroon-700 disabled:opacity-40">{saved ? "Already saved" : result.holes !== 18 ? "18 holes required" : target ? "Link & import" : "Import course"}</button></div>;
    })}</div>
