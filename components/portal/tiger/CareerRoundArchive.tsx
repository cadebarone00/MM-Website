...
export function CareerRoundArchive({ player, year, format, records, teamRecords }: { player: string; year: string; format: string; records: CareerHoleRecord[]; teamRecords: CareerTeamHoleRecord[] }) {
  const [source, setSource] = useState("maroon-masters");
  return <div>
    <div className="mt-4"><Tabs items={[{ value: "maroon-masters", label: "Maroon Masters" }, { value: "other", label: "Other" }]} value={source} onChange={setSource} /></div>
    <RoundArchiveList key={source} player={player} year={year} format={format} records={records.filter((row) => (row.source ?? "maroon-masters") === source)} teamRecords={source === "other" ? [] : teamRecords} />
  </div>;
}

function RoundArchiveList({ player, year, format, records, teamRecords }: { player: string; year: string; format: string; records: CareerHoleRecord[]; teamRecords: CareerTeamHoleRecord[] }) {
  ...
  const rounds = useMemo<ArchiveRound[]>(() => {
    ...
    records.filter((record) => record.player === player && (year === "all" || record.year === Number(year)) && (format === "all" || record.format === format)).forEach((record) => {
      const id = `individual-${careerRoundKey(record)}`;
    ...
    return [
      ...[...individual.entries()].map(([id, holes]) => ({ id, datePlayed: holes[0].datePlayed, kind: "Individual" as const, year: holes[0].year, round: holes[0].round, format: holes[0].format, course: canonicalCourseName(holes[0].course), detail: holes[0].source === "other" ? "Submitted handicap round" : "Individual scorecard", holes })),
  ...
  return <div className="mt-3 grid gap-4 lg:grid-cols-[280px_1fr]">
    <div className="max-h-[520px] overflow-y-auto rounded-sm border border-gold-200 bg-white p-2">
      {rounds.map((round) => <button type="button" key={round.id} onClick={() => setSelectedId(round.id)} className={`mb-1 w-full rounded-sm px-3 py-2 text-left font-sans text-sm ${selected.id === round.id ? "bg-maroon-700 text-white" : "hover:bg-cream-100 text-ink-900"}`}><span className="block font-bold">{round.datePlayed ?? `${round.year} · Round ${round.round}`} · {round.kind}</span><span className="block text-xs opacity-80">{round.format} · {round.course}</span></button>)}
    </div>
    <section className="rounded-sm border border-gold-200 bg-white p-3 sm:p-4">
      ...
      <h3 className="mt-1 font-serif text-xl font-bold text-ink-900">{selected.datePlayed ?? `${selected.year} · Round ${selected.round}`} · {selected.format}</h3>
