...
export function CourseCsvImport({ onSaved, courseId }: { onSaved: (course: LiveCourse) => void; courseId?: string }) {
  ...
  return <div className="mt-4 rounded-lg border border-gold-300 bg-white p-4">
    <h2 className="font-serif text-lg font-bold text-ink-900">{courseId ? "Update course from CSV" : "Import a course CSV"}</h2>
    {courseId && <p className="mt-2 text-sm text-ink-600">Matching tee names are updated; new tee names are added. Blank values keep existing data. Updated tees become unlocked drafts for review. The saved course name stays the same.</p>}
    ...
    <p className="mt-2 font-sans text-xs text-ink-500">{courseId ? "Include tee names to match existing tee sets." : "If names are missing, the filename becomes the course name and the tee set is named Standard."} Blank names on later rows continue the previous course and tee set.</p>
    <p className="mt-2 font-sans text-xs text-ink-500">One course per file, up to 1 MB. Imported tee sets save as unlocked drafts. Review and lock them to make them available for rounds.</p>
    ...
    {imported && <p role="status" className="mt-3 text-sm text-maroon-700">Saved {imported.name}. <Link className="font-bold underline" href={`/portal/admin/course-library/${imported.id}`}>Review, edit, and lock tee sets</Link></p>}
  </div>;
}
