...
export function CourseCsvImport({ course, onSaved }: { course: LiveCourse; onSaved: (course: LiveCourse) => void }) {
  ...
  return <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs">
    <label className="inline-block cursor-pointer rounded border border-maroon-700 px-2 py-1.5 font-bold text-maroon-700">{busy ? "Importing…" : "Import CSV"}<input type="file" accept=".csv,text/csv" disabled={busy} className="sr-only" onChange={async (event) => {
    ...
    <a href="/templates/course-library.csv" download className="underline">CSV template</a>
