      ...
      <section className="rounded-lg border-2 border-maroon-700 bg-cream-50 p-4 sm:p-6">
        ...
        <div className="mt-4 grid gap-3">
          {Object.values(PUBLIC_WAGER_CATALOG).map((market) => {
            ...
            const modelReady = market.modelStatus === "ready";
            return (
              <article key={market.slug} className="rounded-lg border border-gold-300 bg-white p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <span className={published ? "rounded-full bg-emerald-700 px-3 py-1 font-condensed text-2xs font-bold uppercase tracking-wide text-white" : modelReady ? "rounded-full bg-amber-600 px-3 py-1 font-condensed text-2xs font-bold uppercase tracking-wide text-white" : "rounded-full bg-stone-200 px-3 py-1 font-condensed text-2xs font-bold uppercase tracking-wide text-ink-700"}>{published ? "Public" : modelReady ? "Ready to publish" : "Model in design"}</span>
                </div>
                <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2">
                  <Rule label="Applies to" value={market.appliesTo} />
                  <Rule label="Public slot" value={market.publicSlot} />
                </dl>
                {!modelReady && <p className="mt-4 font-sans text-xs leading-5 text-ink-500">The public slot is reserved. We still need to finalize and implement this market&apos;s model before Tiger can submit it.</p>}
                <button type="button" disabled={!databaseReady || !modelReady || published || publishingSlug === market.slug} onClick={() => publishWager(market.slug)} className="mt-4 rounded-lg bg-maroon-700 px-5 py-2 font-condensed text-sm font-semibold uppercase tracking-wide text-white disabled:opacity-50">
                  {published ? "Submitted to Public Wagers" : publishingSlug === market.slug ? "Submitting…" : modelReady ? "Submit Wager" : "Awaiting Model"}
                </button>
              </article>
            );
          })}
