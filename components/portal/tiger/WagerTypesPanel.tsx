...
export function WagerTypesPanel({ initialWagerTypes, databaseReady }: { initialWagerTypes: WagerType[]; databaseReady: boolean }) {
  ...
  async function publishWager(slug: string) {
    setPublishingSlug(slug); setError(null);
    try {
      const res = await fetch("/api/portal/tiger/wager-types/publish", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ slug }) });
      const data = await res.json();
      if (!data.ok) { setError(data.error); return; }
      setWagerTypes((current) => {
        const withoutExisting = current.filter((item) => item.slug !== data.wagerType.slug);
        return [data.wagerType, ...withoutExisting];
      });
    } catch {
      setError("Could not publish this wager.");
    } finally { setPublishingSlug(null); }
  }
  ...
  return (
    <div>
      ...
      <section className="rounded-lg border-2 border-maroon-700 bg-cream-50 p-4 sm:p-6">
        ...
        <p className="mt-1 font-sans text-sm text-ink-600">These are the exact wager experiences that can appear on the public Wagers section. Submit one here to make it visible; odds remain unavailable until its model requirements are met.</p>
        <div className="mt-4 grid gap-3">
          {Object.values(PUBLIC_WAGER_CATALOG).map((market) => {
            const published = wagerTypes.find((type) => type.slug === market.slug)?.isActive;
            return <article key={market.slug} className="rounded-lg border border-gold-300 bg-white p-4">
              <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-serif text-lg font-bold text-ink-900">{market.name}</h3><p className="mt-1 max-w-2xl font-sans text-sm text-ink-600">{market.description}</p></div><span className={published ? "rounded-full bg-emerald-700 px-3 py-1 font-condensed text-2xs font-bold uppercase tracking-wide text-white" : "rounded-full bg-stone-200 px-3 py-1 font-condensed text-2xs font-bold uppercase tracking-wide text-ink-700"}>{published ? "Public" : "Not submitted"}</span></div>
              <dl className="mt-4 grid gap-3 text-sm sm:grid-cols-2"><Rule label="Applies to" value={market.appliesTo} /><Rule label="Public slot" value={market.publicSlot} /></dl>
              <button type="button" disabled={!databaseReady || published || publishingSlug === market.slug} onClick={() => publishWager(market.slug)} className="mt-4 rounded-lg bg-maroon-700 px-5 py-2 font-condensed text-sm font-semibold uppercase tracking-wide text-white disabled:opacity-50">{published ? "Submitted to Public Wagers" : publishingSlug === market.slug ? "Submitting…" : "Submit Wager"}</button>
            </article>;
          })}
        </div>
      </section>
