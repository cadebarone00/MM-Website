  ...
  return <div className="mt-6">
    ...
    <section className="mt-6">
      ...
      <div className="mt-3 grid gap-3 sm:grid-cols-2">
        {courses.map((course) => <article key={course.id} className="rounded-lg border border-stone-300 bg-white p-4"><h3 className="font-serif text-lg font-bold text-ink-900">{course.name}</h3><p className="mt-1 font-sans text-sm text-ink-600">Par {course.holes.reduce((sum, hole) => sum + hole.par, 0)} · {course.holes.reduce((sum, hole) => sum + hole.yards, 0).toLocaleString()} yards</p><p className="mt-1 font-sans text-xs text-ink-500">{course.rating != null ? `Rating ${course.rating}` : "Rating not entered"}{course.slope != null ? ` · Slope ${course.slope}` : ""}</p></article>)}
