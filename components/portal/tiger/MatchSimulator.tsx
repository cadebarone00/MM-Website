...
export function MatchSimulator({ records, courseHoles }: { records: CareerHoleRecord[]; courseHoles: CareerCourseHole[] }) {
  ...
  const result = useMemo(() => format === "Singles" && holesFinished === 0 && a1 !== b1 && course ? calculatePreRoundSinglesOdds({ records, courseHoles, playerA: a1, playerB: b1, course }) : null, [a1, b1, course, courseHoles, format, holesFinished, records]);
  ...
  const available = format === "Singles" && holesFinished === 0;
  return <div className="grid gap-5 xl:grid-cols-[1.15fr_.85fr]">
    <section className="rounded-lg border-2 border-maroon-700 bg-cream-50 p-5"><p className="font-condensed text-2xs font-bold uppercase tracking-wide text-maroon-700">MM Match Simulator · Career Archive only</p><h2 className="mt-1 font-serif text-2xl font-bold text-ink-900">Match Simulator</h2><p className="mt-1 font-sans text-sm text-ink-600">Change any control and the visual updates automatically. Player, format, and course changes reset the match to all square before the first hole.</p>
      ...
      <div className="mt-5 grid gap-4 rounded-md border border-gold-200 bg-white p-4"><Range label="Holes finished" value={holesFinished} min={0} max={18} display={String(holesFinished)} onChange={setHolesFinished} /><div className="grid gap-3 sm:grid-cols-2"><Range label={`${format === "Singles" ? "Player" : "Team"} A status`} value={teamAStatus} min={-9} max={9} display={statusA} onChange={setTeamAStatus} /><Range label={`${format === "Singles" ? "Player" : "Team"} B status`} value={-teamAStatus} min={-9} max={9} display={statusB} onChange={() => undefined} disabled /></div></div>
      {!available && <p className="mt-4 rounded-md bg-gold-100 px-3 py-2 font-sans text-sm text-ink-700">{format !== "Singles" ? `${format} controls are ready; its scoring engine is the next model to validate.` : "Live Singles controls are ready; live remaining-hole odds are the next Singles model to validate."}</p>}
      {available && result && <p className="mt-4 font-sans text-xs text-ink-500">Measure 1 + Measure 2: 20,000 score pairs per {course} hole, then Measure 3 and Measure 4 adjustments across 10,000 full matches. Smallest samples: A {result.measureOneMinimum[0]}/{result.measureTwoMinimum[0]}, B {result.measureOneMinimum[1]}/{result.measureTwoMinimum[1]}.</p>}
    </section>
