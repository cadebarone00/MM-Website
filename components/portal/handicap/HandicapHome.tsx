...
export function HandicapHome({ playerName, summary, team }: { playerName: string; summary: HandicapSummary; team: Team | null }) {
  const [activeTab, setActiveTab] = useState("maroon-masters");
  return (
    <main className="w-full pb-10">
      ...
      <section className="mx-auto max-w-[720px] px-4 pt-6 sm:px-7 sm:pt-8">
        <div className="flex justify-center border-b border-ink-200" role="tablist" aria-label="Handicap scores">
          {[{ id: "maroon-masters", label: "Maroon Masters" }, { id: "overall", label: "Overall" }].map((tab) => (
      ...
      <section id="handicap-scores" role="tabpanel" aria-labelledby={`handicap-tab-${activeTab}`} className="mx-auto mt-6 max-w-4xl px-4 sm:px-6">
        ...
        {activeTab === "maroon-masters" ? (
          <p className="mt-3 font-sans text-sm text-ink-500">Tournament scores are not connected to the handicap tracker yet. View your submitted scores under Overall.</p>
