...
export function HandicapRoundWizard({ courses, recentCourseIds }: { courses: HandicapCourseOption[]; recentCourseIds: string[] }) {
  ...
  if (state.step === "course") {
    return (
      <HandicapCourseLookup
        courses={courses}
        recentCourseIds={recentCourseIds}
        onSelect={(course) => {
          setTeeSetId(course.teeSets[0]?.id ?? "");
          setState({ step: "setup", course });
        }}
      />
    );
  }
      ...
      <div className="mt-4 flex flex-col gap-3">
        <div className="flex items-center justify-between gap-2 rounded-sm border border-ink-200 px-3 py-2">
          ...
          <button
            ...
            className="font-condensed text-xs font-semibold uppercase tracking-wide text-maroon-700"
          >
            Change course
          </button>
        </div>
