...
export function LiveMatchScorecard({ match, scorecard }: { match: RealMatch; scorecard: MatchProfileScorecard | null }) {
  ...
  return <section>{scorecard.courseName && <p className="mb-3 font-condensed text-sm font-bold text-maroon-700">{scorecard.courseName}</p>}<div className={`${styles.scorecard} border-y border-ink-300`} tabIndex={0} aria-label="Match scorecard table"><table className="w-full table-fixed border-collapse"><colgroup><col style={{ width: "var(--match-label-width)" }} />{Array.from({ length: holes.length + 1 }, (_, index) => <col key={index} />)}</colgroup><tbody>
