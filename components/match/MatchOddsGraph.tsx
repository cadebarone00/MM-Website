  ...
  return (
    <section className={styles.timeline} aria-label="Win probability">
      ...
      {latest ? <>
        ...
        {active ? <p className="sr-only">{active.state_thru === 0 ? "Before play" : decided && selectedHole >= decided.thru || final && active === latest ? `Final · Thru ${active.state_thru}` : `Thru ${active.state_thru}`} · Maroon {Math.round(active.maroon_win_probability * 100)}% · Tie {Math.round(active.tie_probability * 100)}% · White {Math.round(active.white_win_probability * 100)}%</p> : <p className="sr-only">{selectedHole === 0 ? "Before play" : `Thru ${selectedHole}`} · No saved odds for this point.</p>}
        <p className="sr-only">{estimateNote ?? "Saved odds by completed hole."} The line balances the two teams’ chances, with ties pulling it toward the middle.</p>
