-- === Player Birdies future ================================================
-- Each player's own birdies (exactly one under par) across every Singles and
-- Fourball round, as Over/Under with alternate lines. See
-- lib/wagers/playerBirdiesFuture.ts.
--
-- Run once in the Supabase SQL editor, AFTER team_winner_future.sql,
-- low_individual_future.sql, hole_in_one_future.sql and
-- total_birdies_future.sql (this file replaces their shared closeout trigger
-- function so it settles all five markets). Safe to re-run.

-- Every published set of Player Birdies lines. The newest row per season is
...
create table if not exists player_birdies_odds_snapshots (
...
create index if not exists player_birdies_odds_snapshots_current_idx
  on player_birdies_odds_snapshots (season_year, created_at desc);

alter table player_birdies_odds_snapshots enable row level security;
drop policy if exists player_birdies_odds_snapshots_select_all on player_birdies_odds_snapshots;
create policy player_birdies_odds_snapshots_select_all on player_birdies_odds_snapshots for select using (true);
-- Writes happen server-side with the service-role key only.

-- Settles player-birdies:<year> once every round is locked, every match is
...
-- graded against that player's own birdie count and that line. Half lines
-- mean no pushes. Safe to call repeatedly; a no-op until final and after
-- settling.
create or replace function settle_player_birdies_if_final(p_year integer) returns void
language plpgsql security definer set search_path = public as $$
declare
  v_market text := 'player-birdies:' || p_year;
...
begin
  ...
  birdies as (
    select r.player_slug, count(h.hole) filter (where h.score = p.par - 1) as total
    from live_roster r
    left join live_hole_scores h
  ...
  winners as (
    ...
    from mm_coin_bets b
    join birdies t on t.player_slug = split_part(b.selection_key, ':', 1)
...
revoke all on function settle_player_birdies_if_final(integer) from public, anon, authenticated;
