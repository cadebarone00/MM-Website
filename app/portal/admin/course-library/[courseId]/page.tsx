...
export default async function EditCoursePage({ params }: { params: Promise<{ courseId: string }> }) {
  ...
  return <main className="mx-auto max-w-4xl px-4 py-8 sm:px-7"><Link href="/portal/admin/course-library" className="font-condensed text-xs font-bold uppercase tracking-wide text-ink-500 hover:text-maroon-700">← Course Library</Link><section className="mt-5 rounded-xl border border-gold-300 bg-cream-50 p-5"><p className="font-condensed text-2xs font-bold uppercase tracking-[0.16em] text-ink-500">Global course</p><h1 className="mt-1 font-serif text-3xl font-bold text-ink-900">{course.name}</h1><p className="mt-2 font-sans text-sm text-ink-600">Create every tee set here. Each has its own yardages, course rating, and slope; individual rounds can mix these tee sets hole by hole.</p>{source && <a href={golfCoreUrl(source.courseId)} target="_blank" rel="noreferrer" className="mt-2 block text-xs underline">Course data: GolfCore</a>}<CourseTeeSetEditor course={course} /></section></main>;
}
