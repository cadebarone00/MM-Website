...
import { refreshGolfTees } from "@/lib/live/golfApiMapping";
...
export async function GET(request: Request) {
  ...
  if (url.searchParams.has("status")) return NextResponse.json({ ok: true, configured: !!process.env.GOLF_API_KEY?.trim() });
...
export async function POST(request: Request) {
  ...
  if (!body || typeof body.providerId !== "string" || !/^[0-9]{1,50}$/.test(body.providerId) || body.courseId !== undefined && typeof body.courseId !== "string") return NextResponse.json({ ok: false, error: "Choose a course from search results." }, { status: 400 });
  const service = createSupabaseServiceRoleClient();
  try {
    const { data: linked, error: linkedError } = await service.from("live_courses").select("id").contains("tee_sets", [{ apiSource: { courseId: body.providerId } }]).limit(1);
    ...
    const hash = createHash("sha256").update(`golfapi:${body.providerId}`).digest("hex");
    ...
    if (previous.some((tee) => tee.apiSource && tee.apiSource.courseId !== body.providerId)) throw new Error("This course is already linked to a different provider course.");
