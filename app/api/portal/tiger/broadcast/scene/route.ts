...
/**
 * Host-only manual scene control (Producer Mode, spec §25). Always acts on
...
export async function POST(request: Request) {
  ...
  const update =
    scene === null || scene === undefined
      ? { automation_mode: "auto", scene_started_at: new Date().toISOString(), updated_at: new Date().toISOString() }
      : { automation_mode: "producer", current_scene: scene, scene_started_at: new Date().toISOString(), updated_at: new Date().toISOString() };
