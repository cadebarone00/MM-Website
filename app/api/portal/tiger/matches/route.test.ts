...
test("POST /api/portal/tiger/matches rejects when requireHost resolves null", async () => {
  ...
  const request = new Request("http://localhost/api/portal/tiger/matches", {
    method: "POST",
    body: JSON.stringify({ session: 1, matchNumber: 1, maroonPlayers: ["cam", "drew"], whitePlayers: ["cade", "collin"] }),
  });
