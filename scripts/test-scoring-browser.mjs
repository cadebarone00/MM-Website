...
createRoot(document.getElementById('root')).render(location.pathname==='/personal'?<HandicapHoleEntry draftKey="test-personal" teeSet={{id:'gold',name:'Gold',rating:72,slope:113,holes}} onBack={()=>{}} onComplete={()=>{document.body.dataset.complete='true'}}/>:<ScoringPanel playerSlug="cade-barone" playerFullName="Cade Barone" round={1} matchBox={box} nameBySlug={{}}/>);`;
...
try{
 ...
 for(let h=1;h<18;h++)await page.getByRole('button',{name:'Next Hole',exact:true}).click();await page.getByRole('button',{name:'Review Round',exact:true}).click();await page.getByRole('alert').waitFor();assert.match(await page.getByRole('alert').innerText(),/Complete putts/);assert.equal(await page.locator('body').getAttribute('data-complete'),null);
 assert.deepEqual(errors,[]);console.log('PASS: personal draft survives reload and incomplete stats prevent review');
