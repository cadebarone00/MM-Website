/** Rebuild 2024 from Pinehurst Player Input plus its corrected round map. */
import fs from "node:fs";
import path from "node:path";
import * as XLSX from "xlsx";
import type { CareerHoleRecord, CareerTeamHoleRecord } from "../lib/data/careerStats";
const source = process.argv[2];
if (!source) throw new Error("Pass the Pinehurst workbook path.");
const archivePath = path.join(process.cwd(), "lib/data/careerArchive.generated.ts");
const mapPath = path.join(process.cwd(), "docs/source-data/2024_Pinehurst_Player_Round_Map_CORRECTED.csv");
type Row = { Player: string; Round: string; Course: string; Format: string; "Partner(s)": string; Scorecard_Sheet: string; Hole_Score_Cells: string };
...
for (const row of map) {
  const player = row.Player.trim().toUpperCase(), partner = row["Partner(s)"].trim().toUpperCase(), round = Number(row.Round), format = row.Format.trim(), course = row.Course.trim();
...
const teamRecords = [...read<CareerTeamHoleRecord[]>(archive, "careerArchiveTeamRecords", "careerArchivePartnerships").filter((row) => row.year !== 2024), ...alternate];
...
for (const [player, rounds] of [...audit].sort(([a], [b]) => a.localeCompare(b))) console.log(player + ": individual " + rounds.join(", ") + "; Alternate Shot 4");
