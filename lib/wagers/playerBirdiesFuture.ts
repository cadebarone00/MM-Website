...
import { birdieChance, birdiesSoFar } from "./totalBirdiesFuture";

/**
 * Player Birdies future: each player's own birdies (exactly one under par)
 * across every individual-ball round (Singles and Fourball), as Over/Under
 * with alternate lines. The public card shows a slider of lines per player,
 * defaulting to the line nearest 50/50. Every line is its own selection
 * ("cam-latto:over:12.5"), so a bet keeps the line it was placed at.
 *
...
export function playerBirdiesMarketKey(seasonYear: number): string {
  return `player-birdies:${seasonYear}`;
}

/** Simulated final totals for one player, as a histogram: histogram[t] = runs ending on t birdies. */
...
/** Every half-birdie line where both Over and Under keep at least MIN_SIDE_PROBABILITY. */
...
export type PlayerBirdiesEntry = { player: string; birdiesSoFar: number; expected: number; featured: number; lines: BirdieLine[] };
...
export function playerBirdiesMarket(seasonYear: number, entries: PlayerBirdiesEntry[], displayName: (player: string) => string): Market {
  const selections = entries.flatMap((entry) =>
    entry.lines.flatMap((line) => [
      { key: playerBirdieSelectionKey(entry.player, "over", line.line), label: `${displayName(entry.player)} over ${line.line} birdies in ${seasonYear}`, odds: line.overOdds },
      { key: playerBirdieSelectionKey(entry.player, "under", line.line), label: `${displayName(entry.player)} under ${line.line} birdies in ${seasonYear}`, odds: line.underOdds },
    ]),
  );
  return {
    marketKey: playerBirdiesMarketKey(seasonYear),
    groupLabel: "Player Birdies",
