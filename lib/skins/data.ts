...
export async function get2026SkinsResults() {
  ...
  const holes: { round_id: string; hole: number; score: number; par: number }[] = [];
  for (let offset = 0; offset < rounds.length; offset += 100) {
    ...
    for (let from = 0; ; from += 1000) {
      const { data, error } = await service.from("archived_scorecard_holes")
        .select("round_id, hole, score, par").in("round_id", ids)
  ...
  return calculateSkinsResults(rounds.map((round) => ({
...
export async function get2026Skins(): Promise<Record<string, number>> {
  return (await get2026SkinsResults()).totals;
}
