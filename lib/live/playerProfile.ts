...
export function playerProfilePayload(snapshot: LiveTournamentSnapshot, player: string): LiveFeedPayload {
  ...
  for (const number of [...new Set(snapshot.matchBoxes.filter(box => box.format !== "Foursome" && [...box.maroonPlayers, ...box.whitePlayers].includes(player)).map(box => box.round))].sort((a, b) => a - b)) {
    ...
    rounds.push({ round: number, course: course.name, format: snapshot.matchBoxes.find(box => box.round === number)?.format,
