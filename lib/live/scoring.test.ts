...
test("summarizePlayer excludes Foursome rounds from individual stats", () => {
  const snapshot = seedSnapshot();
  snapshot.matchBoxes = [
    { id: null, seasonYear: 2027, session: 1, matchNumber: 1, format: "Singles", teeTime: new Date(), maroonPlayers: ["cade"], whitePlayers: ["cam"], state: "Scheduled", started: false },
    { id: null, seasonYear: 2027, session: 2, matchNumber: 1, format: "Foursome", teeTime: new Date(), maroonPlayers: ["cade"], whitePlayers: ["cam"], state: "Scheduled", started: false },
  ];
