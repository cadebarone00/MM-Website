...
export function parseCourseCsv(text: string, fallbackName = "Imported course"): { name: string; teeSets: LiveTeeSet[] } {
  ...
  if (!headers.some((header) => ["course_name", "tee_name", "hole", "par", "yards", "color", "rating", "slope"].includes(header))) throw new Error("No course columns were recognized. Use the downloadable template headers.");
  const tees = new Map<string, LiveTeeSet>();
  let name = "";
  let currentTee = "Standard";
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    currentTee = row.tee_name || currentTee;
    row.tee_name = currentTee;
    const par = row.par ? Number(row.par) : 0, yards = row.yards ? Number(row.yards) : 0;
    ...
    if (row.hole || row.par || row.yards) {
      const number = row.hole ? Number(row.hole) : (tee.holes.at(-1)?.number ?? 0) + 1;
      if (!Number.isInteger(number) || number < 1 || number > 18) fail("Hole must be 1–18. Do not include Out/In/Total rows.");
