...
import { buildScorecardRows, type ScorecardDraftEntry } from "./scorecard.ts";
...
function draftWith(overrides: Record<number, Partial<ScorecardDraftEntry>>): Record<number, ScorecardDraftEntry> {
  const base: ScorecardDraftEntry = { score: "4", putts: "2", fir: true, gir: true, firDirection: null, girDirection: null };
...
test("a hole is not entered until putts is actually set, even though score already has a par default", () => {
  const rows = buildScorecardRows(teeSet, draftWith({ 1: { putts: "" } }));
...
test("a hole is not entered until GIR is set (hit, or a miss direction)", () => {
  const rows = buildScorecardRows(teeSet, draftWith({ 1: { gir: false, girDirection: null } }));
...
test("a non-par-3 hole is not entered until fairway is set, but a par-3 doesn't need it", () => {
  ...
  const rows = buildScorecardRows(teeSet, missingFairway);
...
test("a fully entered hole reports its real score/putts/GIR/fairway", () => {
  const rows = buildScorecardRows(teeSet, draftWith({ 1: { fir: false, firDirection: "left", gir: false, girDirection: "short" } }));
...
test("an entered par-3 hole reports fairway as not applicable instead of the raw default", () => {
  const rows = buildScorecardRows(teeSet, draftWith({}));
